...
and the `pheanstalk examples <https://github.com/AgencyPMG/queue-pheanstalk/tree/master/examples>`_
for code samples that show how everything fits together.
...
- A **producer** adds messages to the queue backend via a *driver* and a
  *router*.
- A **consumer** pulls messages out of the queue via a *driver* and processes
  them with *handlers*.
- A **driver** is a PHP representation of the queue backend. There is an in-memory
